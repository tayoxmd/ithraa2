-- Make public hotel functions callable by anonymous users by using SECURITY DEFINER
-- and setting search_path to public. Also grant execute to anon role explicitly.

-- Recreate get_public_hotels with SECURITY DEFINER
...
-- Recreate get_public_hotel with SECURITY DEFINER
CREATE OR REPLACE FUNCTION public.get_public_hotel(
  p_hotel_id uuid
)
RETURNS TABLE(
  id uuid,
...
GRANT EXECUTE ON FUNCTION public.get_public_hotels(uuid, boolean) TO anon;
GRANT EXECUTE ON FUNCTION public.get_public_hotel(uuid) TO anon;