...
import { UserCircle, Apple, Chrome, Search } from "lucide-react";
...
export function GuestOTPVerification({ onVerified, open, onOpenChange }: GuestOTPVerificationProps) {
  ...
  const handleAppleSignIn = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'apple',
      options: {
        redirectTo: `${window.location.origin}/guest`,
      }
    });
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 pt-4">
            {mode === 'main' && (
              <>
                <Button
                  className="w-full h-14 text-lg gap-3 bg-[#0076ff] hover:bg-[#0066dd] text-white shadow-lg"
                ...
                <Button
                  variant="outline"
                  className="w-full h-14 text-lg gap-3 border-green-600 text-green-600 hover:bg-green-50"
                  onClick={() => {
                    onOpenChange(false);
            ...
            {mode === 'guest' && (
              <>
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={handleGuestContinue} className="flex-1">
